...
""" This is a small app to query the Wargaming Server Info API (https://developers.wargaming.net) in order to collect,
    store, and display their world-wide game server populations """
...
# def parse_data(data: dict, game_abbr: str = None) -> NamedTuple:
    """ Parse data returned from request_data() and return regional totals """
    # Reports = namedtuple("Reports", ["totals", "servers"])
    ...
    # return Reports(totals, servers)
    # return Reports(dict(totals), None)
...
def print_content(game_info: DefaultDict) -> None:
    ...
    # for region, region_data in game_info.totals.items():
